 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      //put in rest of values at t
      AD<double> y0 = vars[y_start + t - 1];
      AD<double> cte0 = vars[cte_start + t - 1];
      AD<double> epsi0 = vars[epsi_start + t - 1];

      //put in values for actuation
      AD<double> delta0 = vars[delta_start + t - 1];
      AD<double> a0 = vars[a_start + t - 1];
      //put in values for cte & epsi calculation
      AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2]*x0*x0 + coeffs[3]*CppAD::pow(x0, 3);
      AD<double> psides0 = CppAD::atan(3*coeffs[3]*x0*x0+ 2*coeffs[2]*x0 +coeffs[1]);

      fg[1 + x_start + t] = x1 - (x0 + v0 * CppAD::cos(psi0) * dt);
      fg[1 + y_start + t] = y1 - (y0 + v0 * CppAD::sin(psi0) * dt);
      fg[1 + psi_start + t] = psi1 - (psi0 - (v0/Lf) * delta0 * dt);   //flip delta sign due to orientation change
      fg[1 + v_start + t] = v1 -(v0 + a0 * dt);
      fg[1 + cte_start + t] = cte1 - ((f0 - y0) + v0 * CppAD::sin(psi0) * dt); //
      fg[1 + epsi_start + t] = epsi1 - ((psi0 - psides0) - v0/Lf * delta0 * dt); //epsi0 or (psi0 - psides0)?
    }
